...
export interface P2PServiceOptions {
	/* Config (should be P2PConfig with node property) */
	config: Config;
...
/**
 * Base class for all P2P services
 * Uses P2PPeerPool instead of PeerPool for libp2p-style networking
 *
 * Key differences from Service:
 * - Uses P2PPeerPool which requires a P2PNode instance
 * - Does not add protocols to server (protocols handled at transport level)
 * - Requires P2PConfig with node property
 *
...
export class P2PService {
	...
	/**
	 * Create new P2P service and associated peer pool
	 */
	constructor(options: P2PServiceOptions) {
		log("Creating P2PService");
		...
		if (!p2pConfig.node) {
			...
			throw new Error(
				"P2PService requires P2PConfig with node property. Use P2PConfig instead of Config.",
			);
		}
	...
	async close() {
		...
		log("Closing P2PService");
		if (this.opened) {
			await this.pool.close();
		}
		this.opened = false;
		log("P2PService closed");
		return true;
	}
