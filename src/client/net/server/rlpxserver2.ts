...
export class RlpxServer extends Server {
	...
	/**
	 * Dial a peer (simplified API that uses dialProtocol internally)
	 * Establishes connection and returns ProtocolStream for the first available protocol
	 *
	 * @param peerIdOrInfo Peer ID (hex string) or PeerInfo object
	 * @param protocol Optional protocol string (defaults to "/eth/68/1.0.0")
	 * @returns Promise resolving to ProtocolStream
	 */
	async dial(
		peerIdOrInfo: string | PeerInfo,
		protocol: string = "/eth/68/1.0.0",
	): Promise<ProtocolStream> {
		return this.dialProtocol(peerIdOrInfo, protocol);
	}

	/**
	 * Connect to a peer discovered via DPT
	 * Uses dialProtocol internally to establish connection
	 */
	private connectToPeer(peer: PeerInfo): void {
		...
		// Use dialProtocol to establish connection
		// Default to eth/68 protocol, but connection will be established regardless
		this.dialProtocol(peer, "/eth/68/1.0.0")
			.then(() => {
				// Connection established via dialProtocol
				// The connection is already handled in dialProtocol
			})
			.catch((err) => {
				...
				if (
					...
					err.message?.includes("Failed to establish connection")
				) {
					this.dpt.banPeer(peer, 300000); // 5 min
				}
				this.error(err);
	 ...
	 *
	 * If peer is not connected, attempts to dial using DPT peer discovery.
	 * Requires either peerId (hex string) or peerInfo (address/port/id).
	 *
	 * @param peerIdOrInfo Peer ID (hex string) or PeerInfo object with address/port/id
	 * @param protocol Protocol string (e.g., "/eth/68/1.0.0")
	 * @returns Promise resolving to ProtocolStream
	 *
	 * @example
	 * ```typescript
	 * // Using peer ID (must be already connected or discoverable via DPT)
	 * const stream = await server.dialProtocol(peerId, "/eth/68/1.0.0");
	 *
	 * // Using peer info (will dial if not connected)
	 * const stream = await server.dialProtocol({
	 *   address: "192.0.2.1",
	 *   tcpPort: 30303,
	 *   id: peerIdBytes
	 * }, "/eth/68/1.0.0");
	...
	async dialProtocol(
		...
		let peerId: string;
		let connection: RLPxConnection | undefined;

		// Handle peerId string or PeerInfo object
		if (typeof peerIdOrInfo === "string") {
			peerId = peerIdOrInfo;
			...
			if (!connection) {
				// Try to find peer in DPT and dial
				if (!this.dpt) {
					throw new Error(
						`Peer ${peerId} not connected and DPT not available for discovery`,
					);
				}

				// Look for peer in DPT
				const peerInfo = this.dpt.getPeer(peerId);
				if (!peerInfo) {
					throw new Error(
						`Peer ${peerId} not found in DPT. Peer must be connected or discoverable.`,
					);
				}

				// Dial the peer
				this.config.logger?.debug(
					`Dialing peer ${peerId.slice(0, 8)}... for protocol ${protocol}`,
				);

				const dialAddr = multiaddr(
					`/ip4/${peerInfo.address}/tcp/${peerInfo.tcpPort}`,
				);

				connection = await this.transport!.dial(dialAddr, {
				...
				// Handle the new connection (creates peer, binds protocols, etc.)
				await this.handleConnection(connection, false);

				// Wait a bit for connection to be fully established
				await new Promise((resolve) => setTimeout(resolve, 100));

				// Verify connection is now in our map
				connection = this.connections.get(peerId);
				if (!connection) {
			...
			// PeerInfo object provided
			const peerInfo = peerIdOrInfo;

			if (!peerInfo.id || !peerInfo.address || !peerInfo.tcpPort) {
				throw new Error("PeerInfo must include id, address, and tcpPort");
			}

			peerId = bytesToUnprefixedHex(peerInfo.id);
			...
			if (!connection) {
				// Dial the peer
				this.config.logger?.debug(
					`Dialing peer ${peerId.slice(0, 8)}... at ${peerInfo.address}:${peerInfo.tcpPort} for protocol ${protocol}`,
				);
		...
		if (!stream) {
			const availableProtocols = connection
				...
				.map(
					(p) =>
						`${p.constructor.name.toLowerCase()}/${(p as any)._version ?? "?"}`,
				)
				.join(", ");
			throw new Error(
