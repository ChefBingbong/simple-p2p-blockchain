...
import { AbstractMessageStream, AbstractMessageStreamInit } from '../stream/default-message-stream'
import { MessageStreamDirection } from '../stream/types'
import { AbortOptions } from './types'

export interface AbstractMultiaddrConnectionInit extends AbstractMessageStreamInit {
...
export abstract class AbstractMultiaddrConnection extends AbstractMessageStream {
  ...
  constructor (init: AbstractMultiaddrConnectionInit) {
    super({
      ...init,
      logNamespace: init.logNamespace ?? `p2p:maconn:${init.remoteAddr.toString()}`
    })
