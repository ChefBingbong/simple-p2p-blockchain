...
import type { TxReceipt } from "../../../vm/index.ts";
...
export class BoundStreamEthProtocol extends EventEmitter {
	...
	private handleBlockHeadersMessage(decoded: any) {
		const [reqIdBytes, headersRaw] = decoded as [
			Uint8Array,
			BlockHeaderBytes[],
		];
		...
		if (headers.length > 0) {
			...
			if (
				!this.updatedBestHeader ||
				latestHeader.number > this.updatedBestHeader.number
			) {
	...
	private handleNewBlockMessage(decoded: any) {
		try {
			...
			if (
				!this.updatedBestHeader ||
				block.header.number > this.updatedBestHeader.number
			) {
			...
			this.config.logger?.error(`Error handling NewBlock: ${err.message}`);
		}
	}
	...
	async getBlockHeaders(opts: {
		...
		try {
			...
			const responsePromise = new Promise<[bigint, BlockHeader[]]>(
				(resolve, reject) => {
					const timeout = setTimeout(() => {
						this.pendingRequests.delete(reqId);
						this.config.logger?.warn(
							`GetBlockHeaders timeout for reqId=${reqId}`,
						);
			...
			const requestPayload = [
				...
				[
					typeof opts.block === "bigint"
						? bigIntToUnpaddedBytes(opts.block)
						: opts.block,
			...
			this.config.logger?.error(`Failed to get block headers: ${err.message}`);
			return undefined;
		}
	}
	...
	async getBlockBodies(opts: {
		...
		try {
			...
			// Create promise for response
			const responsePromise = new Promise<[bigint, BlockBodyBytes[]]>(
				(resolve, reject) => {
					const timeout = setTimeout(() => {
						this.pendingRequests.delete(reqId);
						this.config.logger?.warn(
							`GetBlockBodies timeout for reqId=${reqId}`,
						);
						resolve([reqId, []]); // Timeout returns empty
					}, 10000);
			...
			this.config.logger?.error(`Failed to get block bodies: ${err.message}`);
			return undefined;
		}
	}
	...
	async getPooledTransactions(opts: {
		...
		try {
			...
			// Create promise for response
			const responsePromise = new Promise<[bigint, TypedTransaction[]]>(
				(resolve, reject) => {
					const timeout = setTimeout(() => {
						this.pendingRequests.delete(reqId);
						resolve([reqId, []]);
	...
	async getReceipts(opts: {
		...
		try {
			...
			// Create promise for response
			const responsePromise = new Promise<[bigint, TxReceipt[]]>(
				(resolve, reject) => {
					const timeout = setTimeout(() => {
						this.pendingRequests.delete(reqId);
						resolve([reqId, []]);
			...
			this.config.logger?.error(`Failed to get receipts: ${err.message}`);
			return undefined;
		}
	}
	...
	async request(name: string, args?: any): Promise<any> {
		this.config.logger?.debug(`📤 Request: ${name}`);
		...
		this.config.logger?.warn(`Request type "${name}" not fully implemented`);
		return undefined;
	}
	...
	send(name: string, args?: any): void {
		...
		try {
			...
			if (code === undefined) {
				this.config.logger?.warn(`Unknown message type: ${name}`);
				return;
			}
			...
			this.config.logger?.error(`Error sending ${name}: ${err.message}`);
		}
	}
}
