...
import { isLegacyTx, type TypedTransaction } from "../../../tx/index.ts";
import {
	BIGINT_0,
...
export class StreamEthProtocol extends Protocol {
	...
	private async handleStream(stream: MplexStream) {
		...
		stream.addEventListener("message", async (evt: any) => {
			try {
				...
				this.config.logger?.error(`❌ Error handling message: ${err.message}`);
			}
		});

		stream.addEventListener("close", () => {
			this.config.logger?.info(`📪 ETH stream closed: ${stream.id}`);
		});
	...
	private async handleStatus(stream: MplexStream, decoded: any) {
		...
		this.config.logger?.info(`✅ Peer status validated - handshake complete!`);
	}
		...
		try {
			...
			if (typeof request.block !== "bigint") {
				try {
					...
					this.config.logger?.debug(`Block hash not found, returning empty`);
			...
			this.config.logger?.error(`Error getting headers: ${err.message}`);
		}
		...
		try {
			for (const hash of request.hashes) {
				try {
					...
					bodies.push([
						block.transactions.map((tx) => tx.raw()),
						block.uncleHeaders.map((h) => h.raw()),
					]);
				} catch {
			...
			this.config.logger?.error(`Error getting bodies: ${err.message}`);
		}
	 ...
	 */
	private async handleGetPooledTransactions(stream: MplexStream, decoded: any) {
		const [reqId, hashes] = decoded as [Uint8Array, Uint8Array[]];
	 ...
	 */
	private async sendMessage(stream: MplexStream, code: number, payload: any) {
		try {
	...
	async sendStatus(stream: MplexStream) {
		...
		const protocolVersion = stream.protocol.includes("/eth/68")
			? 68
			: stream.protocol.includes("/eth/67")
				? 67
				: 66;
	...
	async sendBlockBodies(
		...
		await this.sendMessage(stream, StreamEthProtocol.MSG_BLOCK_BODIES, payload);
		// DON'T close stream - keep it open for ongoing communication
	}
	...
	async sendPooledTransactions(
		...
		await this.sendMessage(stream, StreamEthProtocol.MSG_POOLED_TXS, payload);
		// DON'T close stream - keep it open for ongoing communication
	}
