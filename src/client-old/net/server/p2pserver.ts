...
export class P2PServer extends Server {
	...
	private initializeProtocolHandlers() {
		if (!this.registrar) {
			this.config.logger?.error(
				"❌ Registrar not initialized - protocols will not work!",
			);
			return;
		}
	...
	async bootstrap(): Promise<void> {
		...
		const promises = this.bootnodes.map((ma) => {
			...
			this.config.logger?.info(`📡 Bootstrapping to ${host}:${port} (UDP+TCP)`);
	...
	private async handleInboundConnection(connection: Connection) {
		try {
			...
			const address = components.find((c) => c.code === 4)?.value || "unknown";
	...
	private async handleConnection(
		...
		try {
			...
			connection.addEventListener("close", () => {
				...
				this.config.logger?.info(
					`👋 Peer disconnected: ${peerId.slice(0, 8)}...`,
				);
