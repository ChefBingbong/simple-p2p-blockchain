...
export class EVMPerformanceLogger {
  ...
  startTimer(tag: string) {
    if (this.currentTimer !== undefined) {
      throw EthereumJSErrorWithoutCode(
        'Cannot have two timers running at the same time',
      )
    }
  ...
  unpauseTimer(timer: Timer) {
    if (this.currentTimer !== undefined) {
      throw EthereumJSErrorWithoutCode(
        'Cannot unpause timer: another timer is already running',
      )
    }
    timer.unpause()
  ...
  stopTimer(
    ...
    if (this.currentTimer === undefined || this.currentTimer !== timer) {
      throw EthereumJSErrorWithoutCode(
        'Cannot stop timer: another timer is already running',
      )
    }
